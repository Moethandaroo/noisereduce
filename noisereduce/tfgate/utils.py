...
def amp_to_db(x: tf.Tensor, eps: float = 1e-16, top_db: float = 40) -> tf.Tensor:
    """
...
def temperature_sigmoid(x: tf.Tensor, x0: float, temp_coeff: float) -> tf.Tensor:
    """
...
def linspace(start: Number, stop: Number, num: int = 50, endpoint: bool = True, **kwargs) -> tf.Tensor:
    """
    ...
    """
    if endpoint:
        return tf.linspace(start, stop, num, **kwargs)
    else:
        step = (stop - start) / num
        return tf.range(start, stop, step, **kwargs)
